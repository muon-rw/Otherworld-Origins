...
public final class ShapeshiftCameraObstruction {
    ...
    // Only the local player fades, so the smoother is single-instance state driven by that one render call per frame.
    ...
    /**
     * @return blended vertex alpha cap in [{@value #MIN_CAMERA_ENTITY_ALPHA}, 1] smoothed over time for the
     *         local player, 1.0F for any other player
     */
    ...
    private static boolean isLocalPlayer(Player player) {
        ...
        return local != null && local.getId() == player.getId();
    }

    private static float computeTargetAlpha(Player player, Entity fakeEntity, float partialTick) {
        if (player.isSpectator() || !ShoulderSurfingIntegration.isShapeshiftObstructionFadeEnabled()) {
            return 1.0F;
        }
    ...
    private static Vec3 resolveCameraOffsetInBodySpace(Player player, float partialTick) {
        if (ModList.get().isLoaded(SHOULDER_SURFING_ID)) {
            IShoulderSurfing inst = ShoulderSurfing.getInstance();
