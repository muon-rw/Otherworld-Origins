 ...
 * state. Reflection runs once per entity class; per-player {@link AnimationState}
 * instances are cached so the render hot path involves zero reflective access.
 * <p>
 * States are advanced once per source game tick, not once per rendered frame: subclasses such as
 * Naturalist's SmoothAnimationState step a blend factor on every {@code animateWhen} call, so a
 * per-frame drive would make their blend speed frame-rate dependent.
 * <p>
 * Priority (highest first): attack, land, swim, fly, fall, block, run, sit, walk, idle.
 */
public class VanillaAnimationSync {
    ...
    private static final Set<String> ATTACK_KEYWORDS = Set.of(
            ...
            // Naturalist's Elephant (and Mammoth, which extends it) attack through swingAnimationState
            "swing"
    );

    private enum AnimCategory { IDLE, ATTACK, RUN, WALK, SIT, BLOCK, FALL, LAND, FLY, SWIM, SKIP }

    private record TypeConfig(
            @Nullable Field idleField,
            List<Field> attackFields,
            @Nullable Field runField,
            @Nullable Field walkField,
    ...
    private static final Map<Integer, AnimationState> ACTIVE_ATTACK = new HashMap<>();
    ...
    private static final Map<Integer, Integer> LAST_DRIVEN_TICK = new HashMap<>();
    ...
    public static boolean triggerNamedAttack(int entityId, Entity target, String animationKey) {
        ...
        chosen.start(tick);
        ACTIVE_ATTACK.put(entityId, chosen);
     ...
     * the states themselves are advanced only on the first call of each source game tick.
     * <p>
     * Attack animations are selected externally via {@link #triggerNamedAttack} from
     * {@link ShapeshiftRenderHelper}, and held active here for the duration of the attack timer.
     */
    public static void syncAnimations(Entity source, Entity target) {
        ...
        if (!inAttack) {
            ATTACK_END_TICK.remove(id);
            ACTIVE_ATTACK.remove(id);
        }
        if (!inLand) LAND_END_TICK.remove(id);

        if (!consumeDriveTick(id, tick)) return;

        boolean timed = inAttack || inLand;
        boolean swimming = !timed && cache.swim  != null && source.isSwimming();
        boolean flying   = !timed && !swimming && cache.fly   != null && livingSource != null
                && (livingSource.isFallFlying() || (!source.onGround() && target instanceof FlyingAnimal));
        ...
        boolean moving   = livingSource != null && livingSource.walkAnimation.speed() > 0.01F;
        boolean walking  = !timed && !swimming && !flying && !falling && !blocking && !sprinting && !sneaking && cache.walk != null && moving;
        boolean idle     = !timed && !swimming && !flying && !falling && !blocking && !sprinting && !sneaking && !walking;

        AnimationState activeAttack = inAttack ? ACTIVE_ATTACK.get(id) : null;
        for (AnimationState attack : cache.attacks) attack.animateWhen(attack == activeAttack, tick);

        if (cache.land  != null) cache.land.animateWhen(inLand, tick);
        ...
        if (cache.walk  != null) cache.walk.animateWhen(walking, tick);
    ...
    public static void evict(int playerId) {
        ...
        ACTIVE_ATTACK.remove(playerId);
        PREV_ON_GROUND.remove(playerId);
        LAND_END_TICK.remove(playerId);
        LAST_DRIVEN_TICK.remove(playerId);
    }
    ...
    private static boolean consumeDriveTick(int entityId, int tick) {
        Integer prev = LAST_DRIVEN_TICK.put(entityId, tick);
    ...
    private static PlayerCache getOrBuildCache(int playerId, Entity entity) {
        ...
        try {
            ...
            AnimationState walk  = readField(tc.walkField, entity);
            ...
            all.addAll(attacks);
            for (AnimationState s : new AnimationState[]{run, walk, sit, block, fall, land, fly, swim}) {
                if (s != null) all.add(s);
            }
            ...
            PlayerCache cache = new PlayerCache(
                    ...
                    run, walk, sit, block, fall, land, fly, swim, List.copyOf(all));
    ...
    private static TypeConfig getOrBuildTypeConfig(Class<?> clazz) {
        return TYPE_CONFIGS.computeIfAbsent(clazz, c -> {
            ...
            boolean hasAnything = idleField != null || !attackFields.isEmpty()
                    || runField != null || walkField != null || sitField != null || blockField != null
            ...
            return Optional.of(new TypeConfig(
                    ...
                    runField, walkField, sitField, blockField, fallField, landField, flyField, swimField));
        }).orElse(null);
    }
    ...
    private static AnimCategory classify(String fieldName) {
        ...
        if (name.contains("walk") || name.contains("move")) return AnimCategory.WALK;
