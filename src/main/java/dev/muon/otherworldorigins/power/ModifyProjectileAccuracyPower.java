...
public class ModifyProjectileAccuracyPower extends ValueModifyingPowerFactory<ModifyProjectileAccuracyPower.Configuration> {

    public ModifyProjectileAccuracyPower() {
        super(Configuration.CODEC);
    }

    public static float modify(Entity entity, float baseValue) {
        return IPowerContainer.modify(entity, ModPowers.MODIFY_PROJECTILE_ACCURACY.get(), baseValue);
    }

    public static boolean hasPower(Entity entity) {
        return IPowerContainer.get(entity)
                .map(container -> !container.getPowers(ModPowers.MODIFY_PROJECTILE_ACCURACY.get()).isEmpty())
                .orElse(false);
    }

    public static void executeActions(Entity entity) {
        IPowerContainer.get(entity).ifPresent(container -> {
            for (Holder<ConfiguredPower<Configuration, ModifyProjectileAccuracyPower>> holder :
                    container.getPowers(ModPowers.MODIFY_PROJECTILE_ACCURACY.get())) {
